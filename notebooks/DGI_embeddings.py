...
use_embeddings = pd.read_csv(os.path.join('data', 'embeddings_use_large_2000_df.csv'))
use_embeddings.shape

node_content_id_map = {}
for i in range(0, use_embeddings.shape[0], 1):
...
found_dict = {}
not_found_dict = {}
for i in range(0, len(relationship_weights), 1):
...
stellar_G = StellarGraph(G, node_features="v")
print(stellar_G.info())

# https://stellargraph.readthedocs.io/en/stable/api.html#stellargraph.mapper.FullBatchNodeGenerator
fullbatch_generator = FullBatchNodeGenerator(stellar_G, sparse=False, weighted=True)
...
# playing with the embedding vectors
...
text_df = pd.read_csv(os.path.join('data', 'text_use_large_2000_df.csv'))
text_df.head()
...
dgi_embeddings_df.to_csv('dgi_embeddings_df.csv', index=False, header=True, mode='w')
