/*
This query extracts the total number of journeys that engage with the results page (i.e.
selects at least 1 link on the results page) and the two SaB pagePaths that trigger
the banner intervention.

- First, the query creates a table of all disinct sessions that select the banner
  and selects at least 1 link on the results page of the checker (sessions_results_page).
- Next, the query only keeps SaB PAGE hits and the EVENT action 'interventionClicked'
...
- Then, in hit order, the query calculates the previous pagePath of the EVENT action
  ...
  back as only 1 pagePath is the same pagePath (the PAGE hit) as the EVENT hit
  'interactionClicked'
...
-- 'interventionClicked' EVENT hit, the pagePath is the same as one pagePath back
-- (as 1 pagePath back is the PAGE hit). 2 pagePaths back is the PAGE hit before
-- the banner is shown.
