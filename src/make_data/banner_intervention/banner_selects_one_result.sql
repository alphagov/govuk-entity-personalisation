/* This query extracts the total number of sessions that selects the checker via
   the banner intervention that also select at least one link on the results page
*/
...
SELECT
  ...
  (SELECT COUNT(DISTINCT sessionId) FROM sessions_select_result) AS allSessionsThatAccessAtLeastOneResult,
  CAST (100 * (SELECT COUNT(DISTINCT sessionId) FROM sessions_select_result)/COUNT(DISTINCT sessionId) AS NUMERIC) AS proportionOfAllSessionsThatAccessAtLeastOneResult
